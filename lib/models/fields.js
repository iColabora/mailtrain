...
module.exports.create = (listId, field, callback) => {
    ...
    addCustomField(listId, field.name, field.key, field.description, field.defaultValue, field.type, field.group, field.groupTemplate, field.visible, callback);
};
...
function addCustomField(listId, name, key, description, defaultValue, type, group, groupTemplate, visible, callback) {
    type = (type || '').toString().trim().toLowerCase();
    group = Number(group) || null;
    listId = Number(listId) || 0;

    let column = null;
    key = key || slugify('merge ' + name, '_').toUpperCase();
